...
@router.post("/login", response_model=UserRead, response_description="", status_code=status.HTTP_200_OK)
def login(email: str, password: str, db: DBSession) -> str:
    service = AuthService(UserRepository(db))
    token = service.login(email, password)
    return { "token": token, "token_type": "bearer" }

@router.post("/token", response_model=UserRead, response_description="", status_code=status.HTTP_200_OK)
